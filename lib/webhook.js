...
let sendToWebhooks = (eventName) => async (event, campaign_id, sync_all = false) => {
    let webhooks = await db("webhooks").where("campaign_id", campaign_id);
...
module.exports = async function webhook(event_name, modelObj, campaign_id) {

    //emitter.on("new_customer", (customer)=>{
        await sendToWebhooks(event_name)(modelObj, campaign_id, true); 
    //});
}