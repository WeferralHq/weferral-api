        ...
        return new Promise(async (resolve, reject) => {
            ...
            async.parallel({
                participantStats: function (callback) {
                    ...
                    stats.active_participants = stats.invited_participants = stats.flagged_participants = 0;
                    participants.map(participant => {
                        if(participant.data.status === 'active') { stats.active_participants++; }
                        else if(participant.data.status === 'invited') { stats.invited_participants++; }
                        else if(participant.data.status === 'flagged') { stats.flagged_participants++; }
                    });
                    callback(null,stats);
                },
